...
<head>
    ...
    <title>HADLUX - Autorzy postów</title>
</head>
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light fixed-top">
      ...
      <header class="page-header gradient" id="back">  
        <div class="container-fluid pt-5">
            ...
            <h2 class="byc_moze hidden"><b>AUTORZY POSTÓW</b></h2>
                        <section class="authors">

              <div class="box-container byc_moze hidden">
              ...
              <?php
                  ...
                  if($select_author->rowCount() > 0){
                    while($fetch_authors = $select_author->fetch(PDO::FETCH_ASSOC)){ 
                        ...
                        $count_admin_posts = $conn->prepare("SELECT * FROM `posty` WHERE admin_id = ? AND status = ?");
                        $count_admin_posts->execute([$fetch_authors['id'], 'Aktywny']);
                        $total_admin_posts = $count_admin_posts->rowCount();

              ?>
              <div class="box autorzy byc_moze hidden">
                  <p>Autor: <span><?= $fetch_authors['name']; ?></span></p>
                  <p>Liczba postów: <span><?= $total_admin_posts; ?></span></p>
                  <a href="posts.php" class="btn btn-primary byc_moze hidden">Wyświetl posty</a>
              </div>
              <?php
                  }
              }else{
                  echo '<p class="empty">Brak autorów postów</p>';
              }
              ?>
                 </div>
              ...
              <div class="container-fluid pt-5">
            <div class="row align-items-center justify-content-center text-center">
            <h2 class="hidden"><b>OSTATNIO DODANE POSTY</b></h2>
            <section class="posts-grid">
            <div class="box-container">
            <?php
                $select_posts = $conn->prepare("SELECT * FROM `posty` WHERE status = ? ORDER BY id DESC LIMIT 3");
                $select_posts->execute(['Aktywny']);
            ...
            ?>
            <form action="" method="post" class="box byc_moze hidden">
                ...
                <a href="view_post.php?post_id=<?= $post_id; ?>" class="btn btn-primary">Czytaj więcej</a>
                <a class="categoryy" href="category.php?category=<?= $fetch_posts['category']; ?>"><i class="fas fa-tag" style="color: var(--main-color);">&nbsp;</i><?= $fetch_posts['category']; ?></a>
            </form>
            <?php
                    }
                  }
        ...
        <div style="margin-top: 2rem; text-align: center;">
            <a href="posts.php" class="btn btn-primary">Wyświetl wszystkie</a>
        </div>
